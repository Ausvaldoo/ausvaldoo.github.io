...
4. 设置 → 文件与链接 → 删除的文件 → **改成「移到 `.trash/` 文件夹」**
   （⚠️ 不改的话走默认值，删掉的文件会进 **Windows 系统回收站** `E:\$RECYCLE.BIN\...`，不在库里、不好找、还会被系统自动清理。实测 2026-09-09 删的一篇就是掉进那儿了。）
   改完之后：删掉的文件进 `E:\Git_Repos\blog-vitepress\.trash\`，已被 `.gitignore` 排除，**不会发布**。

## 二、新增文章

1. 文件列表右键 `posts` → 新建笔记，命名 `YYYY-MM-DD-标题.md`（如 `2026-09-08-我的新文章.md`）
   - 文件名**建议只用中文 + 数字 + 半角连字符**，别用 `：` `？` `“”` `（）` 这类全角/弯引号标点 —— 能建出来，但生成的网址会被转义成一长串百分号，难看也难分享。
...
- **删除**：右键文件 → 删除（进 `.trash/`，不会发布；若还没改上面的设置，则进 Windows 回收站）
