...
- **在 AI 沙箱里跑爬虫会「启动不起来」（2026-09-21 定位）**。症状：进程卡在
  `[1] 启动浏览器...` 不动、`crawl.log` 只有 8 行、Edge 窗口始终不出现。
  根因**不是** Edge 也不是登录：DrissionPage 的 `PortFinder.__init__` 每次启动都会清理
  `%TEMP%\DrissionPage\autoPortData\` 下的陈旧端口目录，而**本沙箱拦截「删除目录」会永久挂起**
  （`ignore_errors=True` 只能吞异常，吞不掉挂起）。那个 `autoPortData\<port>` 是**上一轮爬取留下的**，
  于是成了自我复现的陷阱：**每跑一次就种一颗雷**。
  绕过办法是**跑之前先把它同卷改名挪走**（不要删）：
  `TS=$(date +%H%M%S); mv "$TEMP/DrissionPage/autoPortData" "$TEMP/DrissionPage/autoPortData_stale_$TS"`
  ⚠️ **跨卷 `mv` 同样不行**（等价于复制 + 删源，删源照样挂）——必须同卷改名。
  站长自己双击 `1_crawl.bat` 属于**非沙箱**运行，删除正常，所以他碰不到这个问题；
  这个坑只在 AI 从沙箱代跑时出现。
  另：`%TEMP%\DrissionPage\userData\` 会随每次全量爬取累积一份完整 Edge 配置
  （2026-09-21 实测 **1,572 MB**，且位于 C 盘）—— 因为 `auto_port` 为真时 DrissionPage
  退出时不删数据。属于可清理的纯垃圾，但**清理同样受上面的删除拦截影响**。
