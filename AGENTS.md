...
**新增/修改的浏览入口**：
- `/tags`（源文件 `tags.md`，导航里叫「标签」）—— **2026-09-21 改成「星座索引」**。
  顶部分类云不变；标签区不再把 23 个标签各自平铺成「标签 + 全部标题」，改为三层：
  ① **频谱** —— 23 个标签按篇数分两档（核心 ≥10 篇 / 其余），点谁聚焦谁；
  ② **聚焦** —— 只渲染当前那一个标签的文章列表，并给出共现排行「常与它一起出现」；
  ③ **邻域星图** —— 默认收起，点「星图」展开；定角布局、可复现，不用力导向。
  理由：旧版把同一批标题重复渲染 3 遍（标签区 202 个链接 + 分类区 64 = 266），
  标签区独占 5 屏；现在首屏链接降到 94 个。
- 首页卡片、归档页、文章底部的分类/标签**都是链接**，点击跳到 `/tags` 的对应锚点
  （分类锚点前缀 `cat-`，标签锚点前缀 `tag-`，例如 `/tags#tag-权力`）。
  写新页面要链接过去时，前缀别写错。

> ⚠️ **改星座索引时必须知道的两件事（2026-09-21）**
> 1. **一次只渲染一个标签**，所以 **23 个 `tag-*` 锚点挂在频谱的 chip 上**（不是挂在文章列表上）。
>    全站 62 处文章底部 `/tags#tag-X` 深链因此依旧有效：落地时 `onMounted` 读 `location.hash`
>    选中该标签；页内点 chip 用 `history.replaceState` 写回 hash（**不是** `location.hash =`，
>    后者会触发一次跳转滚动，点个标签就被弹走半屏）。
> 2. 标签区**换标签需要 JS**（无 JS / 触摸屏 / `prefers-reduced-motion` 时只看到默认那个标签）。
...
动效是「iOS 日期选择器 / 卷轴」式的，三条规则：
1. **光标在 `.wheel` 里的高度比 = 全诗进度**（`target = ratio × maxOffset`）——光标下移，诗向上滚；
2. **离「选中线」最近的那一行被钉在最前**（`scale 1.2` / `opacity 1`），
   其余按距离连续衰减并 `rotateX` 出卷轴曲面；任何时刻**恰好一行**站在最前，不会并列；
3. **鼠标停在哪一行，那一行就浮出英文**（英文在上、中文在下）。
